...
async function startServer() {
  ...
  // Enlarge JSON payload limit for large sessions
  ...
  // API endpoint: Process accumulated session notes with Gemini
  app.post("/api/process-session", async (req, res) => {
    try {
      ...
      // Check for API key (use user-provided or fallback to env variable)
      ...
      // Initialize Google GenAI with the dynamic key
...
`;
  ...
  // Mount Vite middleware of assets in development mode
  ...
  app.listen(PORT, "0.0.0.0", () => {
    console.log(`[NotesNap] Server running on http://localhost:${PORT}`);
  });
}
